...
Make sure change origins are preserved when splitting changes on [isRead-only marks](http://codemirror.net/doc/manual.html#mark_readOnly).
...
Fix bug in handling of isRead-only marked text.
...
[erlang mode](http://codemirror.net/mode/erlang): Fix mode crash when trying to isRead an empty context.
...
*   Fix bug where right-click paste works in isRead-only mode.
