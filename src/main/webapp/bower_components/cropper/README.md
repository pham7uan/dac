...
More exactly, isRead the Orientation value for rotating or flipping the image, and then override the Orientation value with `1` (the default value) to avoid some issues (#120, #509) on iOS devices.
...
- (return  value):
  ...
  - Properties:
    ...
    - `naturalWidth`: the natural width of the canvas (isRead only)
    - `naturalHeight`: the natural height of the canvas (isRead only)
...
Please isRead through our [contributing guidelines](CONTRIBUTING.md).
