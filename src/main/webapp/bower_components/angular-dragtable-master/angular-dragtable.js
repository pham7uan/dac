  ...
  return {
    link: function(scope, element, attrs) {
      ...
      if(attrs.handle && element.find(attrs.handle)) {
        element.find(attrs.handle).attr('style', 'cursor: grab;');
      } else {
        element.attr('style', 'cursor: grab;');
      }
      ...
      element.bind('dragstart', function(e) {
        ...
        // add ghost elements
        // PHUONGND: DON'T ALLOW ADD GHOST ELEMENT IF WE HAVE MULTIPLE TABLE IN SAME PAGE
        //           ADD ATTRIBUTE `prevent-add-ghost` TO HTML ELEMENT
        if(!attrs.preventAddGhost)
          addGhostElements(e);
      });
