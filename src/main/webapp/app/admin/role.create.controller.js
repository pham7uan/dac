(function () {
    ...
    RoleCreateController.$inject = ['$rootScope', '$scope', '$state','$window', 'Role', 'Privilege', 'AlertService', 'AlertModalService', '$translate', 'variables', '$timeout', 'ErrorHandle', 'ComboBoxController', 'User','Principal'];

    function RoleCreateController($rootScope, $scope, $state,$window, Role, Privilege, AlertService, AlertModalService, $translate, variables, $timeout, ErrorHandle, ComboBoxController, User,Principal) {
        var vm = this;
        $scope.ComboBox = {};
        $scope.blockModal;
        $scope.role = {
        ...
        if(Principal.hasAnyAuthority(["ROLE_SYSTEM_ADMIN"]) || Principal.hasAnyAuthority(["ROLE_OWNER"]) || Principal.hasAnyAuthority(["ROLE_ORGANIZATION"])){
            $scope.isAdmin = true;
        }
                    ...
                    if(hasFilter) {
                        if(idFilters.indexOf(Number(selected_nodes[i].key)) != -1) {
                            ...
                            if (existNode == null) {
                                ...
                                if((actionNode.title === 'Chỉ xem thông tin riêng' || actionNode.title.includes("TGT")) && !$scope.isAdmin) continue;
        ...
        $scope.addActionNode = function (node, right_tree, node_remove) {
            ...
            if (existNode == null) {
                ...
                if((actionNode.title === 'Chỉ xem thông tin riêng' || actionNode.title.includes("TGT")) && !$scope.isAdmin) return;
                currentPNode.addChildren(actionNode);
                currentPNode.setExpanded(true);
                currentPNode.getParent().setExpanded(true);
                currentPNode.sortChildren(function (a, b) {
        ...
        $scope.submit = function (isClose) {
            ...
            Role.create($scope.role).then(function (data) {
                ...
                $timeout(function () {
                    isClose ? $state.go('roles') : $state.go('roles-edit',{roleId: data.id});
                },1100);
            })
            .catch(function (data) {
                if($scope.blockModal != null) $scope.blockModal.hide();
