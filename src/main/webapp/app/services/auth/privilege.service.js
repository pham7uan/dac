(function() {
    ...
    function Privilege ($http) {
        ...
        function getAll() {
            return $http.get('/api/privileges/search?query=&size=999').then(function(response) {
                return response.data;
            });
        }

        function getAllForRole(orgId) {
            return $http.get('/api/privileges/search-for-role?orgId='+orgId+'&query=&size=999').then(function(response) {
                return response.data;
            });
        }
